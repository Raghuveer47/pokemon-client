...
const App = () => {
  ...
  useEffect(() => {
    // PART1 Fetch two random Pokémon initially when the component loads
    fetchRandomPokemons();
  }, []);

  const getRandomPokemon = async () => {
    const randomId = Math.floor(Math.random() * 151) + 1; //part2 Fetch random Pokémon from the first 151
  ...
  const getPokemonMove = async (pokemon) => {
    ...
    const validMoves = pokemon.moves.filter(move => move.move.name && move.version_group_details.some(detail => detail.move_learn_method.name === 'level-up'));
    if (validMoves.length === 0) {
      return { name: "Struggle", power: 50 }; // part 4 Default move if no valid moves are available
    }
    const selectedMove = validMoves[Math.floor(Math.random() * validMoves.length)].move;
    ...
    <div className="battle-container">
      ...
      <div className="pokemon-details">
          ...
          <div className="pokemon-block">
            <div className="pokemon-row">
              ...
              <div className="message-body">
                <h3>{pokemon1.name}</h3>
                {showMoves && <p className='upt'>{pokemon1.selectedMove.name}: {pokemon1.selectedMove.power || 'N/A'}</p>}
        ...
        {pokemon2 && (
          <div className="pokemon-block">
            <div className="pokemon-row">
            <img src={pokemon2.sprites.back_default} alt={pokemon2.name} className="pokemon-image" />
              <div className="message-body">
                <h3>{pokemon2.name}</h3>
                {showMoves && <p className='upt2'>{pokemon2.selectedMove.name}: {pokemon2.selectedMove.power || 'N/A'}</p>}
      ...
      <div className="button-container">
        <button className='btn1' onClick={fetchRandomPokemons}>Random Pokémon</button>
        <button  className='btn1' onClick={startBattle}>Start Battle!</button>
      </div>
    </div>
      </div>
  );
